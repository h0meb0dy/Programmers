...
신고자와 피신고자의 관계를 `report_matrix`라는 `n * n` 행렬로 정의해서, index `j`의 이용자가 index `i`의 이용자를 신고했다면 `report_matrix[i][j]`를 `True`로 설정한다.

먼저 각 유저가 몇 번씩 신고당했는지 계산하고, `k`번 이상 신고당했다면 그 유저를 신고한 이용자들이 메일을 받을 횟수(`answer`)를 1 증가시킨다. 이 과정을 반복하면 답을 구할 수 있다.
